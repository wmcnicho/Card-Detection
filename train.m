function [] = train()
    ...
    numberClasses = [2 * ones(8, 1);3 * ones(8, 1); 4 * ones(8, 1);5 * ones(8, 1);6 * ones(8, 1); 7 * ones(8, 1); 8 * ones(8, 1); 9 * ones(8, 1)];
    ...
    eval(['save  trained_models/numbers.mat sNumClass nMeans nInvcors nAprioris']);
